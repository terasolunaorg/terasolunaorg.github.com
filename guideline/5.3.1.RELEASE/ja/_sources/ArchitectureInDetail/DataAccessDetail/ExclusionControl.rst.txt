 ...
 .. note:: **nowaitのサポート**
    ...
    OracleとPostgreSQLについては、\ ``0``\ を指定した場合、\ ``nowait``\ が付加され、他のトランザクションによってロックされていた場合に、ロックの解放待ちを行わずに排他エラーとなる。
     ...
     .. warning:: **PostgreSQL使用時に"nowait"句が付加されない不具合について**
        ...
        TERASOLUNA Server Framework for Java 5.3.1 RELEASE版の依存ライブラリであるHibernate 5.0.X系の不具合(`HHH-10797 <https://hibernate.atlassian.net/browse/HHH-10797>`_\)のため、PostgreSQLを使用した場合、タイムアウト時間を ``0`` に設定してもSQLに"nowait"句が付加されない。
