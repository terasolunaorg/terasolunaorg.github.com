...
.. list-table::
   ...
   * - | (3)
     - | Carry out the settings for accessing the static resource (css, images, js etc.).
       ...
       | ``cache-period="3600"`` is also correct, however, in order to demostrate that it is 60 minutes, it is better to write as ``cache-period="#{60 * 60}"`` which uses `SpEL <http://docs.spring.io/spring/docs/4.1.4.RELEASE/spring-framework-reference/html/expressions.html#expressions-beandef-xml-based>`_ .
   * - | (4)
