...
Basically Boilerplate code is not represent any logic hence it becomes redundant code in implementation.
...
In Lombok, boilerplate code gets generated while compile time
thereby providing a mechanism for removing redundant code from the developers implementation prospective.
...
as classes generated by (the source code output using the auto-generation function of Eclipse) which has about below 60 lines of code instead of 10 lines of code.
...
Initially, introduced a typical annotations that Lombok offers.
...
* Field that holds an object with a mutual reference relationship
...
* The \ ``StackOverflowError`` \ and \ ``OutOfMemoryError`` \ occurs due to recursive referencing
...
.. warning::
    ...
    it is necessary to keep in mind that it tends to the recursive reference.
