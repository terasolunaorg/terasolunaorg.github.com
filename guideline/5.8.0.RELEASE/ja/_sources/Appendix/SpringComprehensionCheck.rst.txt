...
#. Beanの依存関係が以下の図のようになるように(1)～(4)を埋めてください。import文は省略してください。
    ...
    .. figure:: images_SpringComprehensionCheck/appendix-spring_comprehension_check-dependency_relation.png
      :width: 80%
...
#. \ ``@Resource``\ と\ ``@Inject``\ の違いを説明してください
    ...
    .. note::
      ...
      \ ``@Resource``\ は\ ``jakarta.annotation``\ パッケージ、\ ``@Inject``\ は\ ``jakarta.inject``\ パッケージのアノテーションです。
