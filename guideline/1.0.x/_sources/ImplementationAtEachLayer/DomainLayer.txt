...
#. 業務データに対する操作をRepositoryに集約すること、\ **業務データへのアクセスの共通化を行う。**\
...
2. | **Entityを永続化する処理(Repositoryインタフェースの実装クラス)を提供する。**
   ...
   | この役割は、インフラストラクチャ層のRepositoryImplで実装することになる。詳細については、\:doc:`repository`\ を参照されたい。
 ...
 .. list-table::
   ...
   * - | (2)
     ...
     - | 永続先に依存したEntityのCRUD操作の実装を行う。実際のCRUD処理は、Spring Framework、O/R Mapper、ミドルウェアなどから提供されている永続処理用のAPIを利用して行う。
       ...
       | RepositoryImplの実装については、\ :doc:`repository`\ を参照されたい。
...
Serviceの作成単位は主に以下の３パターンとなる。
 ...
 .. warning::
    ...
    **Serviceの作成単位については、開発するアプリケーションの特性や開発体制などを加味して決めて頂きたい。**
    ...
    無秩序にいろいろな単位のServiceを作成する事は避けるべきだが、 **アーキテクトによって方針が示されて状況下においては、併用しても特に問題はない。**
...
\
 ...
 .. note:: **アーキテクチャ観点でのメリット例**
    ...
    #. AOPを使う場合に、JDK標準のDynamic proxies機能が使われる。
       ...
       詳細は、\ `Spring Reference Document <http://static.springsource.org/spring/docs/3.2.x/spring-framework-reference/html/aop.html#aop-proxying>`_\ を参照されたい。
    #. 業務ロジックをスタブ化しやすくなる。
...
- Serviceクラスのメソッドの実装
 ...
 .. list-table::
   ...
   * - 項番
     - 説明
   * - | (1)
     - | 取り寄せ商品が含まれる場合は、\ ``hasOrderProduct``\ を\ ``true``\ が設定する。
   * - | (2)
...
Spring Frameworkから提供されている「宣言型トランザクション管理」の詳細については、\ `Spring Reference Document <http://static.springsource.org/spring/docs/3.2.x/spring-framework-reference/html/transaction.html#transaction-declarative>`_\ を参照されたい。
\
...
\
 ...
 .. note:: **@Transactionalアノテーションを指定する場所**
    ...
    **クラスまたはクラスのメソッドに指定することを推奨する。**
    ...
    理由は、\ `Spring Reference Document <http://static.springsource.org/spring/docs/3.2.x/spring-framework-reference/html/transaction.html#transaction-declarative-annotations>`_\ の2個めのTipsを参照されたい。
 ...
 .. note:: **@Transactionalアノテーションのvalue属性について**
    ...
    複数のTransaction Managerを使う必要がある場合は、\ `Spring Reference Document <http://static.springsource.org/spring/docs/3.2.x/spring-framework-reference/html/transaction.html#tx-multiple-tx-mgrs-with-attransactional>`_\ を参照されたい。
...
.. note:: **org.springframework.transaction.UnexpectedRollbackExceptionが発生する理由**
  ...
  そのため、\ ``SharedService``\ でロールバック対象と例外が発生した場合、\ ``TransactionInterceptor``\ によって、
...
詳細については、\ `Spring Reference Document <http://static.springsource.org/spring/docs/3.2.x/spring-framework-reference/html/transaction.html#transaction-programmatic>`_\ を参照されたい。
