...
Output Escaping
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
| XSSの脆弱性への対応としては、HTML特殊文字をエスケープさせて対策することが基本である。
...
JavaScript Escaping
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
| XSSの脆弱性への対応としては、JavaScript特殊文字をエスケープさせることが基本である。
| 外部からの入力をもとに、JavaScriptの文字列リテラルを動的に生成する場合に、エスケープが必要となる。
...
\
    .. tip::
        ...
        業務要件上必要でない限り、JavaScriptの要素を外部からの入力に依存して動的に生成する仕様は、
