  ...
  .. list-table::
    ...
    * - | (7)
      - | 完了画面を表示するためのハンドラメソッド。
        | \ **GETメソッドでリクエストを受け取る。**\
    * - | (8)
      - | 完了画面を表示するView(JSP/テンプレートHTML)を呼び出し、完了画面を応答する。
        | \ ``.jsp``\ や\ ``.html``\ 等の拡張子は\ ``SpringMvcConfig.java``\ (もしくは\ :file:`spring-mvc.xml`\ )に定義されている \ ``ViewResolver``\ や\ ``TemplateResolver``\ によって付与されるため、ハンドラメソッドの返却値からは省略している。
      ...
      .. code-block:: java
        ...
        public class SpringMvcConfig implements WebMvcConfigurer {
            ...
            public void addInterceptors(InterceptorRegistry registry) {
                registry.addInterceptor(transactionTokenInterceptor()) // (3)
                      .addPathPatterns("/**") // (2)
                      .excludePathPatterns("/resources/**"); // (2)
            }
      ...
      .. list-table::
        ...
        * - | (2)
          - | \ ``HandlerInterceptor``\ を適用するリクエストパスを指定する。
            | 上記例では、\ ``/resources``\ 配下へのリクエストを除く、全てのリクエストに対して適用している。
        * - | (3)
          - | \ ``@TransactionTokenCheck``\ アノテーションを使用して、トランザクショントークンの生成及びチェックを実施するためのクラス(\ ``TransactionTokenInterceptor``\ )を指定する。
        * - | (4)
          - | トランザクショントークンをHidden領域に自動的に埋め込むためのクラス(\ ``TransactionTokenRequestDataValueProcessor``\ )を設定する。
            | JSPはSpring MVCの\ ``<form:form>``\ タグ、Thymeleafは\ ``th:action``\ 属性を使用して設定する。
...
* \ ``ApplicationContextConfig.java``\ (または\ :file:`applicationContext.xml`\ )に定義されている \ ``ExceptionCodeResolver``\
* \ ``SpringMvcConfig.java``\ (または\ :file:`spring-mvc.xml`\ )に定義されている \ ``SystemExceptionResolver``\
...
トランザクショントークンチェックのView(JSP/テンプレートHTML)での利用方法
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

| トランザクショントークンチェックを行う場合、払い出されたトランザクショントークンが、リクエストパラメータとして送信されるようにView(JSP/テンプレートHTML)を実装する必要がある。
| リクエストパラメータとして送信されるようにする方法としては、\ :ref:`setting`\ を行った上で、\ ``<form:form>``\ タグまたは\ ``th:action``\ 属性を使用して自動的にトランザクショントークンをhidden要素に埋め込む方法を推奨する。
...
出力されたHTMLを確認すると、以下となっていることがわかる。
...
- Controller
  ...
  .. list-table::
    ...
    * - | (3)
      - | 入力チェックを行い、確認画面を表示するためのハンドラメソッド。
        ...
        | 遷移先には、View（JSP/テンプレートHTML）を指定する。
    * - | (4)
  ...
  .. warning::
    ...
    \ ``@TransactionTokenCheck``\ アノテーションを定義したハンドラメソッドの遷移先は、View(JSP/テンプレートHTML)を指定する必要がある。
 
    リダイレクト先などのView（JSP/テンプレートHTML）以外を遷移先に指定すると、次の処理でTransactionTokenの値が変わっており、必ずTransactionTokenエラーが発生する。
    ...
    - \ :file:`SpringMvcConfig.java`\
      ...
      .. code-block:: java
        ...
        public class SpringMvcConfig implements WebMvcConfigurer {
            ...
            @Bean
            public TransactionTokenInterceptor transactionTokenInterceptor(){
                return new TransactionTokenInterceptor(1); // (1)
            }
    ...
    - \ :file:`SpringMvcConfig.java`\
      ...
      .. code-block:: java
        ...
        public class SpringMvcConfig implements WebMvcConfigurer {
            ...
            @Bean
            public TransactionTokenInterceptor transactionTokenInterceptor(){
                return new TransactionTokenInterceptor(5); // (1)
            }
...
* HTMLの出力例
  ...
  | JSP/テンプレートHTMLは、\ :ref:`doubleSubmit_how_to_use_transaction_token_check_jsp`\ で用意したJSPと同等のものを用意する。
  | 変更点は以下の通り。

  * JSP

    actionを、\ ``transactionTokenCheckExample``\ から\ ``globalTokenCheckExample``\ に変更

  * Thymeleaf

    \ ``th:action``\ 属性を、\ ``@{/transactionTokenCheckExample}``\ から\ ``@{/globalTokenCheckExample}``\ に変更
