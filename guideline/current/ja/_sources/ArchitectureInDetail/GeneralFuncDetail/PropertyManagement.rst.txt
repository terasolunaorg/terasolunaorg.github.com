...
.. note::
  ...
  本ガイドラインでは、これらの設定値については、プロパティとして管理(プロパティファイルに定義)することを推奨している。

  これらの設定値をプロパティから取得する仕組みにしておくと、設定値を変更する際に、アプリケーション(warファイルやjarファイル)を再ビルドする必要がないため、テスト済みのアプリケーションをプロダクト環境にリリースする事が可能になる。
      ...
      .. list-table::
        ...
        * - | (1)
          - | locationに設定する値は、リソースのロケーションパスを設定すること。
            ...
            | locationの設定値の詳細は、\ `Spring Framework Documentation -Resources- <https://docs.spring.io/spring-framework/docs/6.2.1/reference/html/core.html#resources>`_\ を参照されたい。
...
How to extend
--------------------------------------------------------------------------------
| プロパティ値の取得方法の拡張について説明する。
| プロパティ値の取得方法の拡張は\ ``org.springframework.context.support.PropertySourcesPlaceholderConfigurer``\ クラスを拡張することで実現できる。
