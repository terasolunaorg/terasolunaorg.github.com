...
| しかしながら、SetterやGetterを利用したBeanマッピングを行うと実装が煩雑になり、プログラムの見通しが悪くなるため、本ガイドラインではOSSで利用可能なBeanマッピングライブラリとして \ :url_mapstruct:`MapStruct </>`\ を使用することを推奨する。
...
.. note::
  ...
  開発プロジェクトを\ :url_multi_blank:`ブランクプロジェクト <>`\ から作成すると、上記の設定はpom.xmlに記載されている。
...
| サポートされている型変換については、\ :url_mapstruct_document:`マニュアル -Implicit type conversions- </reference/html/#implicit-type-conversions>`\ を参照されたい。
...
| 詳しくは\ :url_mapstruct_document:`CollectionMappingStrategyのjavadoc </api/org/mapstruct/CollectionMappingStrategy.html>`\ および \ :url_mapstruct_document:`マニュアル -Collection mapping strategies- </reference/html/#collection-mapping-strategies>`\ を参照されたい。
...
.. list-table::
  ...
  * - | \ ``NullValuePropertyMappingStrategy.SET_TO_DEFAULT``\
    - | プロパティの型に対応した、あらかじめ決められたデフォルト値をマッピングする。例えば、Stringの場合は\ ``""``\ （空文字）をマッピングする。
      | その他の型については\ :url_mapstruct_document:`NullValuePropertyMappingStrategyのJavadoc </api/org/mapstruct/NullValuePropertyMappingStrategy.html#SET_TO_DEFAULT>`\ を参照されたい。
...
| サポートしている型については\ :url_mapstruct_document:`マニュアル </reference/html/#implicit-type-conversions>`\ を参照されたい。
  ...
  .. list-table::
    ...
    * - | (1)
      - | \ ``@Default``\ アノテーションが付与されているコンストラクタが使用される。
        | \ ``@Default``\ アノテーションはMapStructから提供されておらず、独自アノテーションを作成する必要がある。アノテーションの作成例は \ :url_mapstruct_document:`マニュアル -Non-shipped annotations- </reference/html/#non-shipped-annotations>`\ を参照されたい。
              ...
              <configuration>
                  ...
                  <annotationProcessorPaths>
                      <path>
                          <groupId>org.mapstruct</groupId>
                          <artifactId>mapstruct-processor</artifactId>
                          <version>${mapstruct.version}</version> <!-- (2) -->
                      </path>
                      <path>
                          <groupId>org.projectlombok</groupId>
                          <artifactId>lombok</artifactId>
                      ...
                      <path>
                          ...
                          <version>${lombok-mapstruct-binding.version}</version> <!-- (3) -->
                      </path>
                  </annotationProcessorPaths>
                  <compilerArgs>
...
.. note::
  ...
  開発プロジェクトを\ :url_multi_blank:`ブランクプロジェクト <>`\ から作成すると、上記のLombokの設定はコメントアウトされた状態でpom.xmlに記載されている。
