...
.. note::
  ...
  ブランクプロジェクトではデフォルトで\ ``HttpSecurity#formLogin``\ (\ ``<sec:form-login/>``\ タグ)と\ ``HttpSecurity#logout``\ (\ ``<sec:logout/>``\ タグ)が設定されており、Spring Securityのフォーム認証が有効となっている。
    ...
    * SpringSecurityConfig.javaの定義例
    ...
    .. list-table::
      ...
      * - 項番
        - 説明
      * - | (1)
        - | \ ``HttpSecurity#formLogin``\ を呼び出すことで、フォーム認証が有効になる。
    
    .. tip:: \ **auto-config属性について**\

      Java Config には、XML Config で設定可能な\ ``auto-config=true``\ に対応する設定は存在していない。

      \ ``auto-config=true``\ に対応する設定は、フォーム認証(\ ``HttpSecurity#formLogin``\ )、Basic認証(\ ``HttpSecurity#httpBasic``\ )、ログアウト(\ ``HttpSecurity#logout``\ )のデフォルト値の使用と同等となる。
        ...
        .. list-table::
          ...
          * - | \ ``httpBasic(Customizer.withDefaults())``\
            - | RFC1945に準拠したBasic認証を行うSecurity Filter(\ ``BasicAuthenticationFilter``\ )が適用される。
              | 詳細な利用方法は、\ :url_spring_security_site:`BasicAuthenticationFilterのJavaDoc </web/authentication/www/BasicAuthenticationFilter.html>`\ を参照されたい。
          * - | \ ``logout(Customizer.withDefaults())``\
            - | ログアウト処理を行うSecurity Filter(\ ``LogoutFilter``\ )が適用される。
              | ログアウト処理の詳細については、「\ :ref:`SpringSecurityAuthenticationLogout`\ 」を参照されたい。
    
      なお、フォーム認証(\ ``HttpSecurity#formLogin``\ )、もしくはBasic認証(\ ``HttpSecurity#httpBasic``\ )は必ず定義する必要がある。これは、ひとつの\ ``SecurityFilterChain``\ 内には、ひとつ以上のAuthentication FilterのBean定義が必要であるという、Spring Securityの仕様を満たすためである。
...
.. tabs::
  ...
  .. group-tab:: Thymeleaf
    ...
    * xxx-web/src/main/webapp/WEB-INF/views/login/loginForm.html
    ...
    .. warning:: \ **リクエストパラメータの存在チェックについて**\
      ...
      \ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Web context namespaces for request/session attributes, etc.- </usingthymeleaf.html#web-context-namespaces-for-requestsession-attributes-etc.>`\ では、リクエストパラメータの存在チェック方法について\ ``param.containsKey``\ が紹介されているが、\ ``org.thymeleaf.context.WebEngineContext.RequestParametersMap`` \ の実装により、\ ``param.containsKey``\  は\ :url_WebEngineContext:`一律true <>`\ が返却されるため、\ ``param.keySet().contains``\ を使用してパラメータの有無を判断する必要がある。なお、リクエストパラメータ以外にセッション(\ ``session.containsKey``\ )、サーブレットコンテキスト(\ ``application.containsKey``\ )についても同様である。
    ...
    * SpringSecurityConfig.javaの定義例
    ...
    .. code-block:: java
      ...
      public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
          ...
          http.authorizeHttpRequests(authz -> authz
                  .requestMatchers(antMatcher("/login/**")).permitAll() // (3)
                  .requestMatchers(antMatcher("/**")).authenticated()); // (4)
    ...
    .. list-table::
      ...
      * - 項番
        - 説明
      * - | (1)
        - | \ ``loginPage``\ にログインフォームを表示するためのパスを指定する。
      ...
      * - | (2)
        - | \ ``loginProcessingUrl``\ に認証処理を行うためのパスを指定する。
...
.. list-table::
  ...
  * - 項番
    - 説明
  * - | (1)
    - | \ ``UserDetails``\ インタフェースと\ ``CredentialsContainer``\ インターフェースを実装したクラスを作成する。
  * - | (2)
  ...
  * - | (6)
    - | \ ``UserDetails``\ に保持されているパスワードの情報を削除する。
      | \ ``CredentialsContainer``\ を実装することで、認証処理完了時に\ ``eraseCredentials()``\ メソッドが呼び出され機密データ（上記例ではパスワード）を削除することができる。
...
.. _AuthenticationExceptionMasking:
    ...
    * SpringSecurityConfig.javaの定義例
      ...
      .. code-block:: java
        ...
        public AuthenticationManager dbUserLoginManager(
                @Qualifier("authenticationProvider") AuthenticationProvider authenticationProvider) throws Exception {
        ...
        public AuthenticationProvider authenticationProvider(
            ...
            DaoAuthenticationProvider authProvider =
                    new DaoAuthenticationProvider(accountUserDetailsService);
            authProvider.setPasswordEncoder(passwordEncoder); // (3)
            return authProvider;
        }
      ...
      .. list-table::
        ...
        * - | (2)
          - | \ ``ProviderManager``\ のコンストラクタ引数に\ ``AuthenticationProvider``\ を定義する。
            | \ ``UserDetailsService``\ に「\ :ref:`SpringSecurityAuthenticationUserDetailsService`\ 」で作成した\ ``AccountUserDetailsService``\ のbeanを指定する。
            | 本定義により、\ ``DaoAuthenticationProvider``\ が有効になる。
        * - | (3)
          - | setPasswordEncoderメソッドを使用して、ログインフォームで指定されたパスワードをハッシュ化するためのクラス(PasswordEncoder)の設定を行う。
            | PasswordEncoderについては\ :ref:`SpringSecurityAuthenticationPasswordHashing`\ を参照されたい。
      ...
      .. list-table::
        ...
        * - | (2)
          - | \ ``<sec:authentication-manager>``\ 要素内に\ ``<sec:authentication-provider>``\ 要素を定義する。
            ...
            | 本定義により、\ ``DaoAuthenticationProvider``\ が有効になる。
        * - | (3)
          - | \ ``<sec:password-encoder>``\ 要素にログインフォームで指定されたパスワードをハッシュ化するためのクラス(PasswordEncoder)の設定を行う。
            | PasswordEncoderについては\ :ref:`SpringSecurityAuthenticationPasswordHashing`\ を参照されたい。
...
.. list-table:: \ **PasswordEncoderの実装クラス**\
  ...
  * - | \ ``Pbkdf2PasswordEncoder``\
    - | PBKDF2アルゴリズムを使用してパスワードのハッシュ化及び照合を行う実装クラス。
      ...
      | 詳細は、\ :url_spring_security_site:`Pbkdf2PasswordEncoderのJavaDoc </crypto/password/Pbkdf2PasswordEncoder.html>`\ を参照されたい。
  * - | \ ``BCryptPasswordEncoder``\
    - | BCryptアルゴリズムを使用してパスワードのハッシュ化及び照合を行う実装クラス。
      | 詳細は、\ :url_spring_security_site:`BCryptPasswordEncoderのJavaDoc </crypto/bcrypt/BCryptPasswordEncoder.html>`\ を参照されたい。
  * - | \ ``Argon2PasswordEncoder``\
    - | Argon2アルゴリズムを使用してパスワードのハッシュ化及び照合を行う実装クラス。
      | 詳細は、\ :url_spring_security_site:`Argon2PasswordEncoderのJavaDoc </crypto/argon2/Argon2PasswordEncoder.html>`\ を参照されたい。
  * - | \ ``SCryptPasswordEncoder``\
    - | SCryptアルゴリズムを使用してパスワードのハッシュ化及び照合を行う実装クラス。
      | 詳細は、\ :url_spring_security_site:`SCryptPasswordEncoderのJavaDoc </crypto/scrypt/SCryptPasswordEncoder.html>`\ を参照されたい。

.. note::

  \ :url_owasp_cheatsheets:`OWASP(Open Web Application Security Project) </Password_Storage_Cheat_Sheet.html>`\ では\ :url_fips:`FIPS <>`\ に準ずるPBKDF2アルゴリズムが推奨されている。
  ...
  \ ``Argon2PasswordEncoder``\ または\ ``SCryptPasswordEncoder``\ を使用する場合は、ブランクプロジェクトのデフォルト設定から変更する必要がある。
    ...
    .. tabs::
      .. group-tab:: Java Config
        ...
        \ ``ApplicationContextConfig.java``\ のコメントアウトを外し、\ ``SCryptPasswordEncoder``\ の定義を有効化する。
...
.. list-table::
  ...
  * - | (2)
    - | \ ``LogoutFilter``\ は、\ ``CompositeLogoutHandler``\ のメソッドを呼び出し、ログアウト処理を行う。
      | \ ``CompositeLogoutHandler``\ 自体はログアウト処理を持たず、機能の利用状態に応じて登録された他の\ ``LogoutHandler``\ 群を保持する。
  * - | (3)
    - | \ ``CompositeLogoutHandler``\ は、認証に関連する情報や、各種トークンを初期化する\ ``LogoutHandler``\ を呼び出す。
      | 後述する「\ **主なLogoutHandlerの実装クラス**\ 」に記載されている\ ``LogoutHandler``\ が実行される。
  * - | (4)
    - | \ ``LogoutFilter``\ は、\ ``LogoutSuccessHandler``\ のメソッドを呼び出し、画面遷移を行う。
      | デフォルトでは\ ``SimpleUrlLogoutSuccessHandler``\ が設定されるが、「\ :ref:`AuthenticationLogoutSuccessResponse`\」で記載されている他の\ ``LogoutSuccessHandler``\ に変更することも可能である。

|

| \ ``LogoutHandler``\ の実装クラスは複数存在し、それぞれ以下の役割をもっている。
| 下記以外の\ ``LogoutHandler``\ の実装クラスについては、「\ :url_spring_security_site:`LogoutHandler(JavaDoc) </web/authentication/logout/LogoutHandler.html>`\ 」の実装クラスを参照されたい。
...
.. list-table:: \ **主なLogoutHandlerの実装クラス**\
  ...
  * - | \ ``CsrfLogoutHandler``\
    - | CSRF対策用トークンの破棄を行うクラス。
      | Spring Securityを利用する場合、デフォルトで登録される\ ``LogoutHandler``\ となる。
  ...
  * - | \ ``CookieClearingLogoutHandler``\
    - | 指定したクッキーを削除するクラス。
      | \ ``HttpSecurity#logout``\ (\ ``<sec:logout />``\ タグ) で設定できる\ ``deleteCookies``\ (\ ``delete-cookies``\ 属性) を指定した場合、\ ``LogoutHandler``\ として登録される。
  * - | \ ``TokenBasedRememberMeServices``\
      | \ ``PersistentTokenBasedRememberMeServices``\
    - | RememberMen認証用のトークンを破棄するクラス。
      | \ :ref:`Remember Me認証機能<SpringSecurityAuthenticationRememberMe>`\ を有効にした場合、\ ``TokenBasedRememberMeServices``\ または、\ ``PersistentTokenBasedRememberMeServices``\ のどちらか一つが\ ``LogoutHandler``\ として登録される。
      | 「\ :url_spring_security_reference:`Hash-Based Token </servlet/authentication/rememberme.html#remember-me-hash-token>`\ 方式のRemember Me認証」を利用する場合は、\ ``TokenBasedRememberMeServices``\ が設定され、「\ :url_spring_security_reference:`Persistent Token </servlet/authentication/rememberme.html#remember-me-persistent-token>`\ 方式のRemember Me認証」を利用する場合は、\ ``PersistentTokenBasedRememberMeServices``\ が設定される。

| \ ``SecurityContextLogoutHandler``\、\ ``CsrfLogoutHandler``\、\ ``LogoutSuccessEventPublishingLogoutHandler``\ は、\ ``LogoutFilter``\ を生成するタイミングでSpring Securityにより自動で設定され、それ以外の\ ``LogoutHandler``\ に関しては機能を有効にした場合にのみ設定される。
| 上記で記載されているログアウト処理を開発者が実装する必要はないが、上記以外のログアウト処理が必要な場合は独自で処理を実装する必要がある。

.. note:: \ **Clear-Site-Dataヘッダの付与**\

  Spring Securityでは、Webサイトの閲覧用データ（クッキー、ストレージ、キャッシュ）を削除するための\ :url_spring_security_reference:`Clear-Site-Dataヘッダ </reactive/exploits/headers.html#webflux-headers-clear-site-data>`\ を付与する\ ``org.springframework.security.web.header.writers.ClearSiteDataHeaderWriter``\ を提供している。
    ...
    * SpringSecurityConfig.javaの定義例
      ...
      .. list-table::
        ...
        * - 項番
          - 説明
        * - | (1)
          - | \ ``HttpSecurity#logout``\ を呼び出すことで、ログアウト処理が有効となる。
...
| Spring Securityは、ログアウト成功時のレスポンスを制御するためのコンポーネントとして、\ ``LogoutSuccessHandler``\ というインタフェースと実装クラスを提供している。
| 下記以外の\ ``LogoutSuccessHandler``\ の実装クラスについては、「\ :url_spring_security_site:`LogoutSuccessHandler(JavaDoc) </web/authentication/logout/LogoutSuccessHandler.html>`\ 」の実装クラスを参照されたい。
...
.. list-table:: \ **主なLogoutSuccessHandlerの実装クラス**\
  ...
  * - | \ ``SimpleUrlLogoutSuccessHandler``\
    - | 指定したパス(\ ``defaultTargetUrl``\ )にリダイレクトを行う実装クラス。
      | 利用する\ ``LogoutSuccessHandler``\ を指定しなかった場合は、デフォルトで\ ``SimpleUrlLogoutSuccessHandler``\ が設定される。
  ...
  * - | \ ``ForwardLogoutSuccessHandler``\
    - | 指定したパスにフォワードを行う実装クラス。
      | フォワードするパスは、\ ``ForwardLogoutSuccessHandler``\ のコンストラクタで指定する必要がある。
...
.. tip:: \ **#authenticationの紹介**\
  ...
  ここでは、\ ``sec:authentication``\ 属性を用いて認証情報が保持するユーザー情報を表示する際の実装例を説明したが、Spring Security Dialectから提供されている\ ``#authentication``\ を用いても、ThymeleafのテンプレートHTMLから認証情報にアクセスする事が可能である。
...
| なお、\ :url_multi_blank:`ブランクプロジェクト <>`\ には\ ``AuthenticationPrincipalArgumentResolver``\ が設定済みである。
    ...
    * SpringSecurityConfig.javaの定義例
    ...
    .. list-table::
      ...
      * - 項番
        - 説明
      * - | (1)
        - | \ ``usernameParameter``\ にユーザー名のリクエストパラメータ名を指定する。
      * - | (2)
        - | \ ``passwordParameter``\ にパスワードのリクエストパラメータ名を指定する。
    ...
    * SpringSecurityConfig.javaの定義例
    ...
    .. list-table::
      ...
      * - 項番
        - 説明
      * - | (1)
        - | \ ``defaultSuccessUrl``\ に認証成功時に遷移するデフォルトのパスを指定する。
    ...
    * SpringSecurityConfig.javaの定義例
    ...
    .. list-table::
      ...
      * - 項番
        - 説明
      * - | (1)
        - | \ ``defaultSuccessUrl``\ の第二引数に\ ``true``\ を指定する。 (第二引数を指定しない場合は\ ``false``\ となる。) 
    ...
    * spring-security.xmlの定義例
    ...
    .. list-table::
      ...
      * - 項番
        - 説明
      * - | (1)
        - | \ ``always-use-default-target``\ 属性に\ ``true``\ を指定する。 (属性を指定しない場合は\ ``false``\ となる。) 
  ...
  .. group-tab:: Java Config
    ...
    * SpringSecurityConfig.javaの定義例
    ...
    .. list-table::
      ...
      * - | (7)
        - | \ ``failureHandler``\ に\ ``authenticationFailureHandler``\ を設定する。
    ...
    * SpringSecurityConfig.javaの定義例
    ...
    .. list-table::
      ...
      * - 項番
        - 説明
      * - | (1)
        - | \ ``logoutUrl``\ を設定し、ログアウト処理を行うパスを指定する。
...
Spring Securityから提供されている\ :url_spring_security_site:`認証プロバイダ </authentication/AuthenticationProvider.html>`\ で対応できない認証要件がある場合は、\ ``org.springframework.security.authentication.AuthenticationProvider``\ インタフェースを実装したクラスを作成する必要がある。
...
\ :ref:`SpringSecurityAuthenticationLoginForm`\ で作成したログインフォームに対して、会社識別子を追加する。
    ...
    * SpringSecurityConfig.javaの定義例
    ...
    .. code-block:: java
      ...
      public AntPathRequestMatcher antPathRequestMatcher() {
          return antMatcher(HttpMethod.POST, "/authentication");
      }
    ...
    .. list-table::
      ...
      * - 項番
        - 説明
      * - | (1)
        - | (2)の\ ``HttpSecurity#addFilterAt``\ タグを使用して\ ``FORM_LOGIN_FILTER``\ を差し替える場合は、\ ``SecurityFilterChain``\ に以下の設定を行う必要がある。
    
          * \ ``HttpSecurity#formLogin``\ は使用できないため、\ ``httpSecurity#exceptionHandling``\ を使用して\ ``AuthenticationEntryPoint``\ を明示的に指定する。
    
      * - | (2)
        - | \ ``HttpSecurity#addFilterAt``\ を使用して\ ``FORM_LOGIN_FILTER``\ を差し替える。
          |
          | \ ``HttpSecurity#addFilterAt``\ の第一引数に拡張したAuthentication Filterのbeanを指定し、第二引数に登録済みのFilterクラスを指定する。
          | 上記設定では\ ``FORM_LOGIN_FILTER``\ に該当する\ ``UsernamePasswordAuthenticationFilter.class``\ を指定している。
      * - | (3)
        - | \ ``SecurityFilterChain``\ の\ ``httpSecurity#exceptionHandling``\ に使用する\ ``AuthenticationEntryPoint``\ のbeanを指定する。
          |
          | ここでは、\ ``HttpSecurity#formLogin``\ を指定した際に使用される\ ``org.springframework.security.web.authentication.LoginUrlAuthenticationEntryPoint``\ クラスのbeanを指定している。
      * - | (4)
      ...
      * - | (5)
        - | \ ``requiresAuthenticationRequestMatcher``\ プロパティに、認証処理を行うリクエストを検出するための\ ``RequestMatcher``\ インスタンスを指定する。
          ...
          | これは、\ ``HttpSecurity#formLogin``\ の\ ``loginProcessingUrl``\ に\ ``/authentication``\ を指定したのと同義である。
      * - | (6)
        - | \ ``authenticationManager``\ プロパティに、\ ``AuthenticationManager``\ のbeanを設定する。
      * - | (6')
              ...
              <map>
                  <entry key="pbkdf2">
                      <bean class="org.springframework.security.crypto.password.Pbkdf2PasswordEncoder"> <!-- (1) -->
                          ...
                          <constructor-arg name="secretKeyFactoryAlgorithm" value="PBKDF2WithHmacSHA256" /> <!-- (5) -->
                      </bean>
                  </entry>
                  <!-- omitted -->
              </map>
...
.. warning:: \ **SecureRandomの使用について**\
    ...
    詳しくは\ :url_javase17:`SecureRandomのjavadoc </docs/api/java.base/java/security/SecureRandom.html>`\ を参照されたい。
...
| 「\ :url_spring_security_reference:`Remember Me認証 </servlet/authentication/rememberme.html>`\ 」とは、Webサイトに頻繁にアクセスするユーザーの利便性を高めるための機能の一つで、ログイン状態を通常のライフサイクルより長く保持するための機能である。
...
| Spring Securityは、「\ :url_spring_security_reference:`Hash-Based Token </servlet/authentication/rememberme.html#remember-me-hash-token>`\ 方式のRemember Me認証」と「\ :url_spring_security_reference:`Persistent Token </servlet/authentication/rememberme.html#remember-me-persistent-token>`\ 方式のRemember Me認証」をサポートしており、デフォルトではHash-Based Token方式が使用される。
...
.. tabs::
  .. group-tab:: Java Config
    ...
    Remember Me認証を利用する場合は、\ ``HttpSecurity#rememberMe``\ を追加する。
    ...
    .. list-table::
      ...
      * - | (2)
        - | \ ``token-validity-seconds``\ 属性に、Remember Me認証用のTokenの有効時間を秒単位で指定する。
          ...
          | 上記以外の属性については、\ :url_spring_security_reference:`Spring Security Reference -The Security Namespace (<remember-me>) - </servlet/appendix/namespace/http.html#nsa-remember-me>`\ を参照されたい。
  ...
  .. group-tab:: XML Config
    ...
    * spring-security.xmlの定義例
    ...
    .. code-block:: xml
      ...
      <sec:http request-matcher="ant">
          <!-- omitted -->
          <sec:remember-me key="spring-test-km/ylnHv"
...
.. warning::
  ...
  \ ``MessageDigestPasswordEncoder``\ は旧式の実装であり、セキュリティ上のリスクが想定されるため非推奨となっている。
...
| 詳細については\ :url_javase17:`Javadoc </docs/api/java.base/java/security/MessageDigest.html>`\ を参照されたい。
|
...
.. note::
  ...
  * {\ ``salt``\ }\ ``hashValue``\
  ...
  パスワードデータの移行については、\ :url_spring_security_site:`MessageDigestPasswordEncoderのJavadoc </crypto/password/MessageDigestPasswordEncoder.html>`\ を参照されたい。
...
\ ``MessageDigestPasswordEncoder``\ でハッシュ化を行う\ ``DelegatingPasswordEncoder``\ のbeanを定義する。
    ...
    .. list-table::
      ...
      * - | (3)
        - | \ ``MessageDigestPasswordEncoder``\ で使用するハッシュアルゴリズムを指定する。
          ...
          | 指定できる値については、\ :url_javase17:`Java暗号化アーキテクチャ標準アルゴリズム名のドキュメント </docs/specs/security/standard-names.html#messagedigest-algorithms>`\ を参照されたい。
      * - | (4)
