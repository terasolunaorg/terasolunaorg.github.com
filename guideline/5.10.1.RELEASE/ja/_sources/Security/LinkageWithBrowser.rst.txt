...
詳しくは\ :url_owasp_header:`Response Headers </#div-headers>`\ を参照されたい。

.. tip:: \ **ブラウザのサポート状況**\

  これらのヘッダに対する処理は、一部のブラウザではサポートされていない。
  
  ブラウザの公式サイトまたは\ :url_owasp_header:`Browser Support </#div-browsersupport>`\ を参照されたい。
...
出力例以外の設定については\ :url_owasp_header:`Response Headers </#div-headers>`\ のX-Frame-Optionsを参照されたい。
...
出力例以外の設定については\ :url_owasp_header:`Response Headers </#div-headers>`\ のStrict-Transport-Securityを参照されたい。
...
.. note:: \ **HTTP Strict Transport Security (HSTS) preload list**\
  ...
  \ :url_caniuse:`主要なブラウザ <>`\ は全て、HSTS preload listに対応している。

  HSTS preload listへのドメインの登録方法は\ :url_hstspreload:`HSTS Preload List Submission </>`\ を参照されたい。
...
出力例以外の設定については\ :url_owasp_header:`Response Headers </#div-headers>`\ のContent-Security-Policyを参照されたい。
...
出力例以外の設定については\ :url_owasp_header:`Response Headers </#div-headers>`\ のReferrer-Policyを参照されたい。
...
出力例以外の設定については\ :url_owasp_header:`Response Headers </#div-headers>`\ のPermissions-Policyを参照されたい。
...
出力例以外の設定については\ :url_owasp_header:`Response Headers </#div-headers>`\ のClear-Site-Dataを参照されたい。
...
.. warning::
  ...
  X-XSS-Protectionヘッダは\ :url_owasp_header:`OWASP </#div-headers>`\ で非推奨となっている。
  ...
  \ :url_owasp_header:`OWASP </#div-headers>`\ ではX-XSS-Protectionヘッダの代わりに\ :ref:`Content-Security-Policyヘッダ<LinkageWithBrowserContentSecurityPolicy>`\ を使用することを推奨している。
...
.. warning::
  ...
  Public-Key-Pinsヘッダは\ :url_owasp_header:`OWASP </#div-headers>`\ で非推奨となっている。
...
.. warning::
  ...
  Feature-Policyヘッダは\ :url_owasp_header:`OWASP </#div-headers>`\ で非推奨となっている。

  \ :url_owasp_header:`OWASP </#div-headers>`\ ではFeature-Policyヘッダの代わりに\ :ref:`Permissions-Policyヘッダ<LinkageWithBrowserPermissionsPolicy>`\ を使用することを推奨している。
...
セキュリティヘッダの詳細については\ :url_spring_security_reference:`Spring Security Reference -Default Security Headers- </servlet/exploits/headers.html#servlet-headers-default>`\ を参照されたい。
...
.. [#fSpringSecurityLinkageWithBrowser2] 各要素で指定できるオプションは\ :url_spring_security_reference:`Spring Security Reference -The Security Namespace (<headers>)- </servlet/appendix/namespace/http.html#nsa-headers>`\ を参照されたい。
