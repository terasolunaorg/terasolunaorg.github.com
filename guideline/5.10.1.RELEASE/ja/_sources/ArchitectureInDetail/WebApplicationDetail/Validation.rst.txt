...
.. list-table::
  ...
  * - 相関項目チェック
    ...
    - | \ :url_spring_reference:`org.springframework.validation.Validator </core/validation/validator.html>`\ インタフェースを実装したValidationクラス
      | または Bean Validation
...
| エラーメッセージの定義方法については、「\ :ref:`ValidationMessageDef`\ 」を参照されたい。
...
.. note:: \ **@GroupSequenceアノテーションについて**\
  ...
  チェック順番を制御するための仕組みとして\ :url_hibernate_validator:`@GroupSequenceアノテーション </reference/en-US/html_single/#_code_groupsequence_code>`\ が提供されているが、この仕組みは以下のような動作になるため、エラーメッセージの出力順序を制御するための仕組みではないという点を補足しておく。
...
| 詳細は\ :ref:`ValidationTypeMismatch`\ を参照されたい。
|

.. _ValidationNestedBeanSingleItemCheck:
  ...
  .. tabs::
        ...
        <body>
            <form:form method="post" action="${pageContext.request.contextPath}/user/create"
                ...
                <div class="form-group">
                    <div class="col-sm-10">
                        <form:errors path="addresses" class="text-danger" /><%-- (1) --%>
                    </div>
                </div>
                <c:forEach items="${ userForm.addresses }" varStatus="status"><%-- (2) --%>
                    <fieldset class="address">
                        <legend id="Address${ status.index }">Address${ f:h(status.index + 1) }</legend>
                        <div class="form-group">
                            <form:label path="addresses[${ status.index }].userName"
                                class="col-sm-2 control-label">UserName</form:label><%-- (3) --%>
                            <div class="col-sm-10">
                                <form:input path="addresses[${ status.index }].userName"
                                    class="form-control" placeholder="UserName" />
                                <form:errors path="addresses[${ status.index }].userName"
                                    class="text-danger" />
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <c:if test="${ status.index > 0 }">
                            <div class="form-group">
                                <div class="col-sm-offset-2 col-sm-10">
                                    <form:button id="remove${ status.index }" name="remove"
                                        value="${ status.index }"
    ...
    .. group-tab:: Thymeleaf
      ...
      .. code-block:: html
        ...
        <body>
            <form method="post" th:action="@{/user/create}" th:object="${userForm}"
                class="form-horizontal">
                <div class="form-group">
                    <div class="col-sm-10">
                        <span id="addresses.errors" th:errors="*{addresses}"
                            class="text-danger"></span><!--/* (1) */-->
                    </div>
                </div>
                <div th:each="addresses,status : ${userForm.addresses}"><!--/* (2) */-->
                    <fieldset class="address">
                        <legend th:id="'Address'+${status.index}"
                            th:text="|Address${status.index + 1}|"></legend>
                        <div class="form-group">
                            <label th:for="|addresses${status.index}.userName|"
                                class="col-sm-2 control-label">UserName</label>
                            <div class="col-sm-10">
                                <input type="text"
                                    th:field="*{addresses[__${status.index}__].userName}"
                                    class="form-control" placeholder="UserName" /><!--/* (3) */-->
                                <span th:id="'addresses'+${status.index}+'.userName.errors'"
                                    th:errors="*{addresses[__${status.index}__].userName}"
                                    class="text-danger"></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label th:for="|addresses${status.index}.postCode|"
                                class="col-sm-2 control-label">PostCode</label>
                            <div class="col-sm-10">
                                <input type="text"
                                    th:field="*{addresses[__${status.index}__].postCode}"
                                    class="form-control" placeholder="PostCode" />
                        ...
                        <div class="form-group">
                            <label th:for="|addresses${status.index}.address|"
                                class="col-sm-2 control-label">Address</label>
                            <div class="col-sm-10">
                                <input type="text"
                                    th:field="*{addresses[__${status.index}__].address}"
                                    class="form-control" placeholder="Address" />
                        ...
                        <div th:if="${status.index > 0}">
                            <div class="form-group">
                                <div class="col-sm-offset-2 col-sm-10">
                                    <button th:id="'remove'+${status.index}" name="remove"
                                        class="btn btn-default" type="submit"
                                        th:value="${status.index}">remove</button>
                                </div>
                            </div>
                        </div>
                    </fieldset>
                ...
                <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                        <button id="add" name="add" class="btn btn-default" type="submit"
  ...
  .. code-block:: java
    ...
    public class UserController {
        ...
        public UserForm setUpForm() {
            ...
            ArrayList<AddressForm> addresses = new ArrayList<>();
        ...
        public String handleAdd(UserForm form, Model model) {
            ArrayList<AddressForm> addresses = form.getAddresses();
            addresses.add(new AddressForm());
        ...
        public String handleRemove(UserForm form, Model model, Integer remove) {
            ArrayList<AddressForm> addresses = form.getAddresses();
            if (remove != null && remove >= 0 && remove < addresses.size()) {
                addresses.remove(remove.intValue());
            }
            model.addAttribute("addresses", addresses);
            return "user/createForm";
        }
...
.. note::
  ...
  入力フィールドの追加/削除は、JavaScriptで実行することもできる。JavaScriptで実行する場合のコード例については、\ :ref:`ValidationWhenAddingRemovingInputFieldsUsingJavaScript`\ を参照されたい。
  ...
  .. tabs::
    ...
    .. group-tab:: Thymeleaf
      ...
      .. note:: \ **#ids.prevメソッドについて**\
        ...
        \ ``#ids``\ の詳細については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -IDs- </usingthymeleaf.html#ids>`\ を参照されたい。
  ...
  .. list-table::
    ...
    * - | (6)
      - | エラーメッセージのコード名を指定する。ここではコードを、
        ...
        | とする。メッセージ定義は\ :ref:`ValidationMessageInApplicationMessages`\ を参照されたい。
    * - | (7)
...
.. _ValidationHowToCrossFieldValidationForMultiFieldHighlight:
    ...
    .. list-table::
      ...
      * - | (2)
        - | エラーメッセージのコード名を指定する。この際、対応するエラーメッセージに空文字を指定する。
          | メッセージ定義は\ :ref:`ValidationMessageInApplicationMessages`\ を参照されたい。
      * - | (3)
        - | エラーメッセージをコードで解決できなかった場合に使用する、デフォルトメッセージを設定する。
          | 上記の例では空文字を設定している。
...
\ :ref:`ValidationCustomConstraint`\ にて説明する。

|

.. _ValidationMessageDef:
...
#. | \ ``org.springframework.context.MessageSource``\ に定義されているメッセージの中に、ルールに合致するものがあればそれをエラーメッセージとして使用する (Springのルール)。
   | Springのデフォルトのルールについては、「\ :url_spring_javadoc:`DefaultMessageCodesResolverのJavaDoc </org/springframework/validation/DefaultMessageCodesResolver.html>`\ 」を参照されたい。
...
.. warning::
  ...
  * Bean Validation用の共通部品をjarファイルとして配布する際に、\ ``ValidationMessages.properties``\ ファイルをjarファイルの中に含めないように注意すること。
  ...
  \ :url_multi_blank:`ブランクプロジェクト <>`\ からプロジェクトを生成した場合は、\ ``xxx-web/src/main/resources``\ の直下に\ ``ValidationMessages.properties``\ が格納されている。
...
ValidationMessages.propertiesを用意しない場合は、\ :ref:`Hibernate Validatorが用意するデフォルトメッセージ <ValidationDefaultMessageInHibernateValidator>`\ が使用される。

\ ``MessageSource``\ と連携することで、日本語メッセージをNative to Asciiせずに直接扱うことができる。詳細は、\ :ref:`ValidationWithoutNative2Ascii`\ を参照されたい。

|

.. _ValidationMessageInValidationMessages:
...
\ :ref:`ValidationBasicValidation`\ の初めに使用した、以下のフォームを用いて説明する。
      ...
      * - | (1)
        - メッセージの中の 「\ ``${inclusive == true ? 'or equal to ' : ''}``\ 」の部分がEL式である。
          ...
          * must be less than {value}
          ...
          Bean ValidationにおけるEL式の扱いについては、\ :url_hibernate_validator:`Hibernate Validator Reference Guide(Interpolation with message expressions) </reference/en-US/html_single/#section-interpolation-with-message-expressions>`\ を参照されたい。
    ...
    .. list-table::
      ...
      * - | (1)
        - | 上記のメッセージ定義から実際に生成されるメッセージは、 \ ``${validatedValue}``\ の部分にフォームに入力した値が埋め込まれる。
          ...
          |
          | 詳細については、\ :url_hibernate_validator:`Hibernate Validator Reference Guide(Interpolation with message expressions) </reference/en-US/html_single/#section-interpolation-with-message-expressions>`\ を参照されたい。

|

.. _ValidationMessageInApplicationMessages:
...
\ :ref:`ValidationMessageInValidationMessages`\ の設定がある前提で、以下の設定で\ ``email``\ と\ ``age``\ フィールドのメッセージを上書きする。
...
| となる。
| 仕様の詳細については\ :url_spring_javadoc:`SpringValidatorAdapterのJavaDoc </org/springframework/validation/beanvalidation/SpringValidatorAdapter.html#getArgumentsForConstraint(java.lang.String,java.lang.String,jakarta.validation.metadata.ConstraintDescriptor)>`\ を参照されたい。
...
.. note::
  ...
  application-messages.propertiesのメッセージキーの形式は\ :url_spring_javadoc:`これ以外にも用意されている </org/springframework/validation/DefaultMessageCodesResolver.html>`\ が、デフォルトメッセージを一部上書きする目的で使用するのであれば、基本的に、\ ``アノテーション名.フォーム属性名.プロパティ名``\ 形式でよい。

|

.. _ValidationCustomConstraint:
...
.. _ValidationCombineExistingConstraint:
...
* 「年齢」の制限を規定する\ ``@Age``\ アノテーションの実装例
  ...
  .. note::
    ...
    詳細は、\ :url_hibernate_validator:`Hibernate Validatorのドキュメント </reference/en-US/html_single/#section-boolean-constraint-composition>`\ を参照されたい。

|

.. _ValidationImplementNewConstraint:
...
.. _ValidationCannotExpressExisting:
...
.. _ValidationCorrelationItemCheck:

相関項目チェックルール
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

| \ :ref:`ValidationCorrelationCheck`\ で説明したように、Bean Validationによって複数のフィールドにまたがる相関項目チェックを実装できる。
...
.. tip::
  ...
  共通ライブラリでは、2つのフィールドの内容を比較する相関項目チェックの実装として\ :ref:`@Compare <ValidationTerasolunaGfwList>`\ アノテーションを用意している。

  \ `@Compare`\ アノテーションを利用することで、このルールをより簡単に実現することができる。 詳細は\ :ref:`ValidationTerasolunaGfwHowToExtend`\ を参照されたい。
  ...
  .. list-table::
    ...
    * - | (4)
      - | 独自\ ``ConstraintViolation``\ オブジェクトを生成する。
        ...
        | 詳細は、\ :url_jakarta_validation_apidocs:`ConstraintValidatorContextのJavaDoc </validation/constraintvalidatorcontext>`\ を参照されたい。

.. note::

  Spring Validatorによる相関項目チェックにて紹介したように、Bean Validationにおいても\ :ref:`相関チェック対象の複数フィールドに対してエラー情報を設定する<ValidationHowToCrossFieldValidationForMultiFieldHighlight>`\ ことが可能である。
...
.. _ValidationMethodValidationOnSpringFrameworkHowToUseSettings:
...
.. _ValidationMethodValidationOnSpringFrameworkHowToUseApplyTarget:
...
「\ :ref:`ValidationMethodValidationOnSpringFrameworkHowToUseSettings`\ 」を行っただけでは、Method Validationを実行するAOPは適用されない。Method Validationを実行するAOPを適用するためには、インタフェース又はクラスに\ ``@ org.springframework.validation.annotation.Validated``\ アノテーションを付与する必要がある。
...
.. list-table::
  ...
  * - | (1)
    - Bean Validationの制約アノテーションをメソッドの引数アノテーションとして指定する。
      ...
      | 引数にNull値もしくは空文字が指定された場合、\ ``jakarta.validation.ConstraintViolationException``\ が発生する。
  * - | (2)
    - Bean Validationの制約アノテーションをメソッドアノテーションとして指定する。

      上記例では、返り値がNull値もしくは空文字にならないことを示しており、返り値としてNull値もしくは空文字が返却された場合、\ ``jakarta.validation.ConstraintViolationException``\ が発生する。
...
.. _ValidationMethodValidationOnSpringFrameworkHowToUseExceptionHandling:
...
.. warning::
  ...
  Springの機能によるメッセージ補完については、\ :ref:`ValidationMessageInValidationMessages`\ のNoteを参照されたい。

  \ ``ConstraintViolation``\ の詳細については、\ :url_hibernate_validator:`Hibernate Validatorのリファレンス </reference/en-US/html_single/#section-constraint-violation-methods>`\ を参照されたい。
...
| 検証に使用することができるアノテーションのリストは、\ :url_hibernate_validator:`こちら </reference/en-US/html_single/#validator-defineconstraints-spec>`\ を参照されたい。

|

.. _ValidationJsr380Doc:
...
詳細は、\ :url_jakarta_validation:`Bean Validation specification(Built-in Constraint definitions) <#builtinconstraints>`\ を参照されたい。
...
.. warning::
  ...
  E-mailの形式は\ :url_ietf_rfc:`RFC2822 </rfc2822.txt>`\ で定義されているが、\ ``@Email``\ は厳密にRFC2822に準拠していることをチェックするものではない。

  例えばマルチバイト文字（全角文字）を含んでいても\ ``@Email``\ でのチェックをパスすることが確認されている。また、実際に利用されているEmailアドレスも、必ずしもRFC2822に厳密に準拠しているわけではない。

  これらの注意点を考慮した上で、利用・サポートするSMTPサーバなどによって適切なルールでの入力チェックを実装することを推奨する。実装の際は、\ :ref:`ValidationCombineExistingConstraint`\ を参照されたい。
...
.. note::
  ...
  \ ``ClockProvider``\ を実装したクラスの例に関しては、\ :url_hibernate_validator:`Hibernate Validator Reference Guide(ClockProvider and temporal validation tolerance) </reference/en-US/html_single/#section-clock-provider>`\ を参照されたい。

|

.. _ValidationValidatorList:
...
詳細は、\ :url_hibernate_validator:`Hibernate Validator仕様 </reference/en-US/html_single/#validator-defineconstraints-hv-constraints>`\ を参照されたい。
...
.. tip::
  ...
  * アプリケーション全体の\ ``@URL``\ のチェックルールを変更してもよい場合には、\ :url_hibernate_validator:`JavaDoc </api/org/hibernate/validator/constraints/URL.html>`\ に記載されているように、XMLにてValidatorクラスを\ ``RegexpURLValidator``\ に変更する。
  * 一部の項目だけに正規表現による検証を適用し、\ ``@URL``\ はデフォルトのルールを使用したい場合には、新規アノテーション、および\ ``RegexpURLValidator``\ と同様の検証を行う\ ``jakarta.validation.ConstraintValidator``\ 実装クラスを作成し、必要な項目に作成したアノテーションによる検証を適用する。
  ...
  XMLによるチェックルール変更の詳細については\ :url_hibernate_validator:`Hibernateのリファレンス </reference/en-US/html_single/#section-configuration-validation-xml>`\ を、新規アノテーションの作成方法については、\ :ref:`ValidationImplementNewConstraint`\ をそれぞれ参照されたい。

|

.. _ValidationDefaultMessageInHibernateValidator:
...
| 各言語に対応したValidationMessages.propertiesについては \ :url_hibernate_validator_github:`org/hibernate/validator <>`\ を参照されたい。
...
\ :url_gfw:`terasoluna-gfw-common </terasoluna-gfw-common-libraries/terasoluna-gfw-common>`\ が提供するアノテーション(\ ``org.terasoluna.gfw.common.codelist.*``\ )を以下に示す。
...
\ :url_gfw:`terasoluna-gfw-codepoints </terasoluna-gfw-common-libraries/terasoluna-gfw-codepoints>`\ が提供するアノテーション(\ ``org.terasoluna.gfw.common.codepoints.*``\ )を以下に示す。なお、\ ``terasoluna-gfw-codepoints``\ はバージョン5.1.0.RELEASE以上で利用することができる。
...
.. _ValidationTerasolunaGfwList:

terasoluna-gfw-validatorのチェックルール
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

\ :url_gfw:`terasoluna-gfw-validator </terasoluna-gfw-common-libraries/terasoluna-gfw-validator>`\ が提供するアノテーション(\ ``org.terasoluna.gfw.common.validator.constraints.*``\ )を以下に示す。なお、\ ``terasoluna-gfw-validator``\ はバージョン5.1.0.RELEASE以上で利用することができる。
...
.. note:: \ **相関項目チェックにおける入力必須について**\
  ...
  なお、入力フィールドが未入力の場合に \ ``null``\ がバインドされる場合のみ、 \ ``requireBoth``\ 属性が利用できる。Spring MVCでは文字列の入力フィールドに未入力の状態でフォームを送信した場合、デフォルトでは、フォームオブジェクトに\ ``null``\ ではなく、空文字がバインドされることに注意しなければならない。  文字列フィールドが未入力の場合に、空文字ではなく、\ ``null``\ をフォームオブジェクトにバインドするには、\ :ref:`ValidationStringTrimmerEditor`\ を参照されたい。
...
.. _ValidationDefaultMessageInTerasolunaGfw:
...
| 対応する言語については各ライブラリ（\ :url_gfw:`terasoluna-gfw-commonのjar </terasoluna-gfw-common-libraries/terasoluna-gfw-common/src/main/resources>`\ 、\ :url_gfw:`terasoluna-gfw-codepointsのjar </terasoluna-gfw-common-libraries/terasoluna-gfw-codepoints/src/main/resources>`\ 、\ :url_gfw:`terasoluna-gfw-validatorのjar </terasoluna-gfw-common-libraries/terasoluna-gfw-validator/src/main/resources>`\ ）を参照されたい。
...
.. _ValidationTerasolunaGfwHowToUse:
...
あとは\ :ref:`ValidationBasicValidation`\ で説明したように、JavaBeanのプロパティにアノテーションを付与すればよい。
...
.. note::
  ...
  詳細は、\ :url_jakarta_validation:`Bean Validation specification(Exception model) <#exception>`\ を参照されたい。

|

.. _ValidationTerasolunaGfwHowToExtend:
...
以下では、\ :ref:`ValidationCorrelationItemCheck`\ で独自に実装した\ ``@Confirm``\ アノテーションを、共通ライブラリで提供しているチェックルールを利用して作成する例を紹介する。

\ :ref:`ValidationCombineExistingConstraint`\ で説明したように、\ ``@Compare``\ を利用して\ ``@Confirm``\ アノテーションを作成する。
...
.. note::
  ...
  「\ :ref:`ValidationCombineExistingConstraint`\ 」では\ ``@ReportAsSingleViolation``\ を付与する方法を紹介しているが、\ ``@ReportAsSingleViolation``\ を付与するとラップされた\ ``@Compare``\ のエラーメッセージは使用されず、\ ``@Confirm``\ のエラーメッセージのみが表示される。\ ``@Confirm``\ はフォームオブジェクトに対する入力チェックであるため、エラーメッセージはフォームオブジェクトに割り当てられ、実際に表示したい\ ``field``\ 属性に指定したフィールドには割り当てられない。
...
\ :ref:`ValidationCorrelationItemCheck`\ で実装したアノテーションの代わりに、上記で作成したアノテーションを使用する。
...
.. list-table::
  ...
  * - | (3)
    - | \ ``confirmPassword``\ フィールドには、\ ``@NotEmpty``\ アノテーションを付与する。

|

.. _ValidationTypeMismatch:
...
| \ :ref:`ValidationMessageInApplicationMessages`\ で説明したように、\ ``{0}``\ でフィールド名を埋めることができる。
| \ **基本的にデフォルトメッセージは定義しておくこと**\ 。

.. tip::

  メッセージキーのルールの詳細は、\ :url_spring_javadoc:`DefaultMessageCodesResolverのJavaDoc </org/springframework/validation/DefaultMessageCodesResolver.html>`\ を参照されたい。

|

.. _ValidationStringTrimmerEditor:
...
.. _ValidationWithoutNative2Ascii:
  ...
  .. group-tab:: Java Config
    ...
    * Bean定義
      ...
      .. list-table::
        ...
        * - | (4)
          - | \ :ref:`ValidationMethodValidation`\ を利用する際には、\ ``MethodValidationPostProcessor``\ の\ ``validator``\ プロパティに(1)で定義したBeanを指定する。  
...
.. tip::
  ...
  詳細は、OWASPの\ :url_owasp_community:`解説ページ </attacks/Command_Injection>`\ などを参照されたい。
...
エラーの意味を明確にし、かつ、ロジックを隠蔽するために、application-messages.propertiesに適切なメッセージを定義する。メッセージの定義方法については、\ :ref:`ValidationMessageInApplicationMessages`\ を参照されたい。
...
.. _ValidationWhenAddingRemovingInputFieldsUsingJavaScript:

入力フィールドの追加/削除をJavaScriptで実行する場合
--------------------------------------------------------------------------------

\ :ref:`ValidationNestedBeanSingleItemCheck`\ のコレクションに対するバリデーションの場合について、add/removeボタンを押下してAddress入力フィールドを追加/削除する処理を、サーバサイドではなくJavaScriptで実行した場合のコード例を以下に示す。

* Controllerクラス

  \ :ref:`ValidationNestedBeanSingleItemCheck`\ にある\ ``UserController``\ から\ ``handleAdd``\ メソッドと\ ``handleRemove``\ メソッドを削除する。
    ...
    .. group-tab:: JSP
      ...
      .. code-block:: jsp
        ...
        <head>
            ...
            <script type="text/javascript" src="${pageContext.request.contextPath}/resources/app/js/AddressesView.js"></script> <%-- (1) --%>
        </head>
        <body>
            <form:form method="post" action="${pageContext.request.contextPath}/user/create" modelAttribute="userForm" class="form-horizontal">
                <%-- omitted --%>
                <c:forEach items="${ userForm.addresses }" varStatus="status">
                    <fieldset class="address">
                        <%-- omitted --%>
                        <c:if test="${ status.index > 0 }">
                            <div class="form-group">
                                <div class="col-sm-offset-2 col-sm-10">
                                    <form:button id="remove${ status.index }" name="remove" data-action="remove-address" class="btn btn-default">remove</form:button> <%-- (2) --%>
                                </div>
                            </div>
                        </c:if>
                    </fieldset>
                ...
                <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                        <form:button id="add" name="add" data-action="add-address" class="btn btn-default">add</form:button> <%-- (3) --%>
                    </div>
                </div>
                <%-- omitted --%>
            </form:form>
      ...
      .. list-table::
        ...
        * - 項番
          - 説明
        * - | (1)
          - | 使用するJavaScriptファイルを読み込んでいる。
        * - | (2)
          - | カスタムデータ属性（\ ``data-action``\ ）を設定することでremoveボタン押下時にJavaScriptの\ ``removeAddress``\ が実行される。
        * - | (3)
          - | カスタムデータ属性（\ ``data-action``\ ）を設定することでaddボタン押下時にJavaScriptの\ ``addAddress``\ が実行される。
    ...
    .. group-tab:: Thymeleaf
      ...
      .. code-block:: html
        ...
        <head>
            ...
            <script type="text/javascript" th:src="@{/resources/app/js/AddressesView.js"></script> <!-- (1) -->
        </head>
        <body>
            <form method="post" th:action="@{/user/create}" th:object="${userForm}" class="form-horizontal">
                <!-- omitted -->
                <div th:each="addresses,status : ${userForm.addresses}">
                    <fieldset class="address">
                        <!-- omitted -->
                        <div th:if="${status.index > 0}">
                            <div class="form-group">
                                <div class="col-sm-offset-2 col-sm-10">
                                    <button th:id="'remove'+${status.index}" name="remove" data-action="remove-address" class="btn btn-default" type="submit" value="Submit">remove</button> <!-- (2) -->
                                </div>
                            </div>
                        </div>
                    </fieldset>
                ...
                <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                        <button id="add" name="add" data-action="add-address" class="btn btn-default" type="submit" value="Submit">add</button> <!-- (3) -->
                    </div>
                </div>
                <!-- omitted -->
            </form>
...
* JavaScript
  ...
  .. code-block:: javascript
    ...
    function addressTemplate(number) {
        return `
    <fieldset class="address">
        <legend id="Address${number}">Address${number + 1}</legend>
        <div class="form-group">
            <label for="addresses${number}.userName" class="col-sm-2 control-label">UserName</label>
            <div class="col-sm-10">
                <input id="addresses${number}.userName" name="addresses[${number}].userName" placeholder="UserName" class="form-control" type="text" />
            </div>
        </div>
        <div class="form-group">
            <label for="addresses${number}.postCode" class="col-sm-2 control-label">PostCode</label>
            <div class="col-sm-10">
                <input id="addresses${number}.postCode" name="addresses[${number}].postCode" placeholder="PostCode" class="form-control" type="text" />
            </div>
        </div>
        <div class="form-group">
            <label for="addresses${number}.address" class="col-sm-2 control-label">Address</label>
            <div class="col-sm-10">
                <input id="addresses${number}.address" name="addresses[${number}].address" placeholder="Address" class="form-control" type="text" />
            </div>
        </div>
        <div class="form-group">
            <div class="col-sm-offset-2 col-sm-10">
                <button id="remove${number}" data-action="remove-address" class="btn btn-default">remove</button>
            </div>
        </div>
    </fieldset>
    <br />
    ...
    class AddressesView {
        constructor() {
            this.addressSize = $("fieldset.address").length;
        }

        addAddress() {
            const newHtml = addressTemplate(this.addressSize);
            const $lastFieldset = $("fieldset.address").last();

            if ($lastFieldset.length > 0) {
                // Insert after the last <br>
                $lastFieldset.next("br").after($(newHtml));
            } else {
                // If no fieldset exists, console a warning
                console.warn("No existing fieldset found");
            }

            this.addressSize++;
        }

        removeAddress($fieldset) {
            $fieldset.next().remove(); // remove <br>
            $fieldset.remove(); // remove <fieldset>
            this.addressSize = $("fieldset.address").length; // Update size after removal

            // Reindex all fieldsets after removal
            this.reindexAddresses();
        }

        reindexAddresses() {
            $("fieldset.address").each(function (index) {
                const $fieldset = $(this);

                // Update legend text and id
                $fieldset
                    .find("legend")
                    .text(`Address${index + 1}`)
                    .attr("id", `Address${index}`);

                // Update id, name, and for attributes for each input field
                // UserName
                $fieldset.find(`input[name^="addresses"][name$=".userName"]`).attr({
                    id: `addresses${index}.userName`,
                    name: `addresses[${index}].userName`,
                });
                $fieldset.find(`label[for^="addresses"][for$=".userName"]`).attr("for", `addresses${index}.userName`);

                // PostCode
                $fieldset.find(`input[name^="addresses"][name$=".postCode"]`).attr({
                    id: `addresses${index}.postCode`,
                    name: `addresses[${index}].postCode`,
                });
                $fieldset.find(`label[for^="addresses"][for$=".postCode"]`).attr("for", `addresses${index}.postCode`);

                // Address
                $fieldset.find(`input[name^="addresses"][name$=".address"]`).attr({
                    id: `addresses${index}.address`,
                    name: `addresses[${index}].address`,
                });
                $fieldset.find(`label[for^="addresses"][for$=".address"]`).attr("for", `addresses${index}.address`);

                // Remove button
                $fieldset.find('button[data-action="remove-address"]').attr("id", `remove${index}`);
            });
        }
    }
    ...
    $(function () {
        ...
        $('[data-action="add-address"]').click(function (e) {
            e.preventDefault();
            addressesView.addAddress();
        });

        // Use event delegation to handle dynamically added elements as well
        $(document).on("click", '[data-action="remove-address"]', function (e) {
            e.preventDefault();

            const $this = $(this); // this button
            const $fieldset = $this.closest("fieldset.address"); // Get the closest fieldset

            if ($fieldset.length > 0) {
                addressesView.removeAddress($fieldset);
            } else {
                console.error("Could not find parent fieldset");
            }
        });
    });
