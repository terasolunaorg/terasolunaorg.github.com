...
指定するリクエストパスは、具体的な値ではなくパターンを指定することも可能である。パターン指定の詳細は、\ :url_spring_reference:`Spring Framework Documentation -URI patterns- </web/webmvc/mvc-controller/ann-requestmapping.html#mvc-ann-requestmapping-uri-templates>`\ を参照されたい。
...
- 処理毎にリクエストURLを設ける。
  ...
  .. note::
    ...
    Entity参照、Entity更新、Entity削除処理のURL内に指定している\ ``{id}``\ は、\ :url_spring_reference:`URI patterns </web/webmvc/mvc-controller/ann-requestmapping.html#mvc-ann-requestmapping-uri-templates>`\ と呼ばれ、任意の値を指定する事ができる。
...
ハンドラメソッドの引数について
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
\ :url_spring_reference:`ハンドラメソッドの引数は様々な値をとることができる </web/webmvc/mvc-controller/ann-methods/arguments.html>`\ が、基本的には次に挙げるものは原則として使用しないこと。
  ...
  .. group-tab:: JSP
    ...
    - HTML of created by View(hello.jsp)
      ...
      .. list-table::
        ...
        * - | (3)
          - | \ ``addAttribute``\ メソッドの第一引数を省略すると\ :url_spring_javadoc:`Conventions#getVariableName </org/springframework/core/Conventions.html#getVariableName(java.lang.Object)>`\ の仕様に基づき、値のクラス名から属性名を決定する。
     ...
     .. list-table::
        ...
        * - | (2)
          - | \ ``RedirectAttributes``\ オブジェクトの\ ``addFlashAttribute``\ メソッドを呼び出し、渡したいデータを\  ``RedirectAttributes``\ オブジェクトに追加する。
            | 例では、 ``hello`` という属性名で ``HelloWorld!`` という文字列のデータを追加している。
        * - | (3)
          - | \ ``addFlashAttribute``\ メソッドの第一引数を省略すると\ :url_spring_javadoc:`Conventions#getVariableName </org/springframework/core/Conventions.html#getVariableName(java.lang.Object)>`\ の仕様 に基づき、値のクラス名から属性名を決定する。
     ...
     .. list-table::
        ...
        * - | (2)
          - | \ ``RedirectAttributes``\ オブジェクトの\ ``addFlashAttribute``\ メソッドを呼び出し、渡したいデータを\ ``RedirectAttributes``\ オブジェクトに追加する。
            | 例では、\ ``hello``\ という属性名で\ ``Hello World!``\ という文字列のデータを追加している。
        * - | (3)
          - | \ ``addFlashAttribute``\ メソッドの第一引数を省略すると\ :url_spring_javadoc:`Conventions#getVariableName </org/springframework/core/Conventions.html#getVariableName(java.lang.Object)>`\ の仕様に基づき、値のクラス名から属性名を決定する。
            | 例では、\ ``model.addFlashAttribute("helloBean", new HelloBean());``\ を行ったのと同じ結果となる。
        * - | (4)
          - | 画面(View)を直接表示せず、次の画面を表示するためのリクエストにリダイレクトする。
        * - | (5)
  ...
  .. list-table::
    ...
    * - | (2)
      - | \ ``ResponseCookie``\ オブジェクトを生成し、\ ``HttpServletResponse``\ オブジェクトの\ ``Set-Cookie``\ ヘッダに追加する。
        ...
        | 詳しくは、\ :url_spring_javadoc:`ResponseCookie </org/springframework/http/ResponseCookie.html>`\ 及び\ :url_spring_javadoc:`ResponseCookieBuilder </org/springframework/http/ResponseCookie.ResponseCookieBuilder.html>`\ を参照されたい。

.. note::

  HTTP Cookieの処理を規定するRFC 6265では、Cookieの名前や値に一部使用できない文字があることに注意されたい。
    
  \ :url_rfc:`RFC 6265(HTTP State Management Mechanism)の4.1 SetCookie </rfc6265#section-4.1>`\ のSyntaxを参照されたい。
...
ハンドラメソッドの返り値について
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
\ :url_spring_reference:`ハンドラメソッドの返り値についても様々な値をとることができる </web/webmvc/mvc-controller/ann-methods/return-types.html>`\ が、基本的には次に挙げるもののみを使用すること。
...
| Spring Frameworkでは、HTML formから送信されたリクエストパラメータ(文字列)を、フォームオブジェクトに定義されている型に変換してからバインドする機能を提供しているため、フォームオブジェクトに定義するフィールドの型は、\ ``java.lang.String``\ だけではなく、任意の型で定義することができる。
  ...
  .. tip:: \ **Spring Frameworkから提供されている型変換を行う仕組みについて**\
    ...
    * \ :url_spring_reference:`Spring Type Conversion </core/validation/convert.html>`\
    * \ :url_spring_reference:`Spring Field Formatting </core/validation/format.html>`\
    * \ :url_spring_reference:`java.beans.PropertyEditor implementations </core/validation/beans-beans.html#beans-beans-conversion>`\
...
\ ``@NumberFormat``\ アノテーションで指定できる属性は以下の通り。
  ...
  .. list-table::
    ...
    * - 1.
      - style
      - 数値のスタイルを指定する。詳細は、\ :url_spring_javadoc:`NumberFormat.StyleのJavadoc </org/springframework/format/annotation/NumberFormat.Style.html>`\ を参照されたい。
    * - 2.
      - pattern
      - Javaの数値形式を指定する。詳細は、\ :url_javase17:`DecimalFormatのJavadoc </docs/api/java.base/java/text/DecimalFormat.html>`\ を参照されたい。
  ...
  .. list-table::
    ...
    * - | (1)
      - HTML formから送信されるリクエストパラメータの日時形式を指定する。例では、patternとして\ ``yyyyMMdd``\ 形式を指定している。
        リクエストパラメータの値が\ ``20131001``\ の場合、フォームオブジェクトのbirthDateには 2013年10月1日のLocalDateオブジェクトがバインドされる。
...
\ ``@DateTimeFormat``\ アノテーションで指定できる属性は以下の通り。
  ...
  .. list-table::
    ...
    * - 1.
      - iso
      - ISOの日時形式を指定する。詳細は、\ :url_spring_javadoc:`DateTimeFormat.ISOのJavadoc </org/springframework/format/annotation/DateTimeFormat.ISO.html>`\ を参照。
    * - 2.
      - pattern
      - Javaの日時形式を指定する。詳細は、\ :url_javase17:`SimpleDateFormatのJavadoc </docs/api/java.base/java/text/SimpleDateFormat.html>`\ を参照されたい。
    * - 3.
      - style
  ...
  .. note::
    ...
    \ |framework_name|\ではJSR-310 Date And Time APIの使用を推奨する。
...
| このようなメソッドのことを本ガイドラインではModelAttributeメソッドと呼び、\ ``setUpXxxForm``\ というメソッド名で定義することを推奨する。
  ...
  .. code-block:: java
    ...
    public SampleForm setUpSampleForm(
            ...
            @CookieValue(value = "birthDate", required = false) LocalDate birthDate) {
        SampleForm form = new SampleForm();
        form.setName(name);
        form.setAge(age);
...
.. tabs::
  .. group-tab:: JSP
    ...
    | \ ``<form:xxx>``\ タグの詳細は、\ :url_spring_reference:`Spring Framework Documentation -Spring's form tag library- </web/webmvc-view/mvc-jsp.html#mvc-view-jsp-formtaglib>`\ を参照されたい。
...
本章では代表的なJSPタグライブラリの使い方は説明しているが、全てのJSPタグライブラリの説明はしていないので、詳細な使い方については、それぞれのドキュメントを参照すること。
  ...
  .. list-table::
    ...
    * - 1.
      ...
      - - \ :url_spring_reference:`/web/webmvc-view/mvc-jsp.html#mvc-view-jsp-formtaglib`\
    * - 2.
      - Spring's JSP Tag Library
      - - \ :url_spring_reference:`/web/webmvc-view/mvc-jsp.html#mvc-view-jsp-tags`\
    * - 3.
      - JSTL
      - - \ :url_jakarta_tag:`Jakarta Standard Tag Library 3.0 Specification Document <>`\
    * - 4.
...
- web.xml
  ...
  .. note::
    ...
    ディレクティブの詳細は、\ :url_jakarta_pages:`Jakarta Server Pages 3.1 Specification Document(1.10. Directives) <#directives>`\ を参照されたい。

  .. note::

    <jsp-property-group>要素の詳細は、\ :url_jakarta_pages:`Jakarta Server Pages 3.1 Specification Document(3.3. JSP Property Groups) <#jsp-property-groups>`\ を参照されたい。
...
JSTLのJSPタグライブラリから提供されている\ ``<c:out>``\ タグを使用して表示する。
  ...
  .. note::
    ...
    \ ``<c:out>``\ の詳細は、\ :url_jakarta_tag:`Jakarta Standard Tag Library 3.0 Specification Document(4 General-Purpose Actions) <#general-purpose-actions-core-tag-library>`\ を参照されたい。
...
.. note::
  ...
  \ ``<fmt:formatNumber>``\ の詳細は、\ :url_jakarta_tag:`Jakarta Standard Tag Library 3.0 Specification Document(9 Formatting Actions) <#formatting-actions-i18n-capable-formatting-tag-library>`\ を参照されたい。
    ...
    * - | (3)
      - \ ``spring:mvcUrl``\ 関数の引数には、呼び出すControllerのメソッドに割り振られているリクエストマッピング名を指定する。
        ...
        * \ ``buildAndExpand``\ メソッドは、Controllerのメソッドの引数として宣言されていない動的な部分(パス変数など)に埋め込む値を指定した上で、リクエストURLを生成するためのメソッドである。
        ...
        \ ``arg``\ メソッドと\ ``buildAndExpand``\ メソッドの具体的な使用例については、\ :url_spring_reference:`Spring Framework Documentation -Links in Views- </web/webmvc/mvc-uri-building.html#mvc-links-to-controllers-from-views>`\ を参照されたい。
...
.. note::
  ...
  \ ``<form:form>``\ 、\ ``<form:xxx>``\ タグの詳細は、\ :url_spring_reference:`Spring Framework Documentation -Spring's form tag library- </web/webmvc-view/mvc-jsp.html#mvc-view-jsp-formtaglib>`\ を参照されたい。
...
.. note::
  ...
  詳細は、\ :url_jakarta_tag:`Jakarta Standard Tag Library 3.0 Specification Document(5. Conditional Actions) <#conditional-actions-core-tag-library>`\ を参照されたい。
  ...
  .. list-table::
    ...
    * - | (2)
      - | \ ``<c:forEach>``\ タグのvarStatus属性で指定した変数から現在処理を行っている要素位置(count)を取得している。
        | count以外の属性については、\ :url_tomee_javadoc:`jakarta.servlet.jsp.jstl.core.LoopTagStatusのJavaDoc </servlet/jsp/jstl/core/LoopTagStatus.html>`\ を参照されたい。
    * - | (3)
      - | \ ``<c:forEach>``\ タグのvar属性で指定した変数に格納されているオブジェクトから値を取得している。

.. note::

  詳細は、\ :url_jakarta_pages:`Jakarta Standard Tag Library 3.0 Specification Document(6. JSP Documents) <#JSP_Documents>`\ を参照されたい。
...
本章では、ThymeleafおよびThymeleaf + Springのダイアレクト、ならびにThymeleafのSpring Security連携用ダイアレクトで提供されている代表的な属性やオブジェクトの使い方を説明しているが、全てについて説明はしていないので、詳細な使い方については、それぞれのドキュメントを参照すること。
  ...
  .. list-table::
    ...
    * - 1.
      - Thymeleafのダイアレクト
      - - \ :url_thymeleaf_tutorial:`/usingthymeleaf.html`\
    * - 2.
      - Thymeleaf + Springのダイアレクト
      - - \ :url_thymeleaf_tutorial:`/thymeleafspring.html#the-springstandard-dialect`\
    * - 3.
      - ThymeleafのSpring Security連携用ダイアレクト
      - - \ :url_thymeleaf_github:`/thymeleaf-extras-springsecurity`\
...
| 通常は、テンプレートHTMLのどこでもThymeleaf独自の属性を使用できるように、\ ``<html>``\ 要素にネームスペースを付与することを推奨する。
  ...
  .. note::
    ...
    詳細については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -A multi-language welcome- </usingthymeleaf.html#a-multi-language-welcome>`\ を参照されたい。
...
|
  ...
  .. note::
    ...
    詳細については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Numbers- </usingthymeleaf.html#numbers>`\ を参照されたい。
...
\ ``java.util.Date``\ オブジェクトのフォーマットを行う場合は、\ ``#dates.format``\ メソッドを利用する。
  ...
  .. note::
    ...
    詳細については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Dates- </usingthymeleaf.html#dates>`\ を参照されたい。
...
\ ``java.util.Calendar``\ オブジェクトのフォーマットを行う場合は、\ ``#calendars.format``\ メソッドを利用する。
  ...
  .. note::
    ...
    詳細については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Calendars- </usingthymeleaf.html#calendars>`\ を参照されたい。
...
* 現在のパスからの相対パスを指定する
  ...
  .. note::
    ...
    リンクURL式\ ``@{}``\ は、コンテキストルートからの相対パスや現在のパスからの相対パスを生成するほか、サーバールートからの相対パス、プロトコルからの相対パスも生成できる。

    詳細については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Link URLs- </usingthymeleaf.html#link-urls>`\ を参照されたい。
...
* パラメータとして変数を埋め込む
  ...
  .. code-block:: html
    ...
    <form th:action="@{/user/details(userId=${user.id})}" method="post"> <!-- (5) -->
        <!-- omitted -->
    </form>
...
\ ``#mvc.url``\ メソッドを使用すると、Controllerのメソッドのメタ情報(メソッドシグネチャやアノテーションなど)と連携して、リクエストURLを組み立てる事ができる。
  ...
  .. code-block:: html
    ...
    <form th:action="${(#mvc.url('HC#hello')).build()}" method="post"> <!-- (6) -->
        <!-- omitted -->
    </form>
    ...
    * - | (6)
      - | \ ``#mvc.url``\ メソッドの引数には、呼び出すControllerのメソッドに割り振られているリクエストマッピング名を指定する。
        ...
        * \ ``buildAndExpand``\ メソッドは、Controllerのメソッドの引数として宣言されていない動的な部分(パス変数など)に埋め込む値を指定した上で、リクエストURLを生成するためのメソッドである。
        ...
        | 「\ :url_spring_reference:`Spring Framework Documentation -Links in Views- </web/webmvc/mvc-uri-building.html#mvc-links-to-controllers-from-views>`\ 」を参照されたい。
...
| 演算子の詳細については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Standard Expression Syntax- </usingthymeleaf.html#standard-expression-syntax>`\ を参照されたい。
...
条件式には、等価演算子のほかに、算術演算子や比較演算子も利用できる。
  ...
  .. note::
    ...
    詳細については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Aggregates- </usingthymeleaf.html#aggregates>`\ を参照されたい。
      ...
      .. list-table::
        ...
        * - | (2)
          - | \ ``th:each``\ 属性の左項で2つ目に指定した変数から、現在処理を行っている要素の位置を取得している。countは、要素の位置を1始まりで取得している。
            | count以外の属性については、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Keeping iteration status- </usingthymeleaf.html#keeping-iteration-status>`\ を参照されたい。

  .. note::

    \ ``th:each``\ 属性の詳細は、\ :url_thymeleaf_tutorial:`Tutorial: Using Thymeleaf -Iteration- </usingthymeleaf.html#iteration>`\ を参照されたい。
...
.. warning::
  ...
  \ ``Map.get(key)``\ は\ ``Map[key]``\ と書くこともできる。ただし、\ ``Map[key]``\ でキーに式を指定する場合、式に「.」を含む場合は式として評価されるが、含まない場合はキーに指定した式を式ではなく文字列として評価してしまい、「\ :url_spring_framework_issues:`意図したキーを指定できないバグ </20447>`\ 」の影響を受ける。
...
| 詳しくは\ :ref:`BlankProjectMavenCompilerPlugin`\ を参照されたい。
...
.. caution::
  ...
  当節で説明しているSuffix MatchやTrailing Slashの使用は、Spring MVCでは\ :url_spring_github:`registered-suffixes-only </spring-webmvc/src/main/resources/org/springframework/web/servlet/config/spring-mvc.xsd#L63-L76>`\ や\ :url_spring_github:`trailing-slash </spring-webmvc/src/main/resources/org/springframework/web/servlet/config/spring-mvc.xsd#L52-L58>`\ に記載されている通り非推奨となっており、パスマッチングのデフォルト設定を変更する方法(\ ``registered-suffixes-only``\ や\ ``trailing-slash``\ )はいずれ削除される可能性がある。
...
| ここでは、\ :url_spring_reference:`Spring Framework Documentation -Suffix Match- </web/webmvc/mvc-controller/ann-requestmapping.html#mvc-ann-requestmapping-suffix-pattern-match>`\ に記載されている通り、登録された特定の拡張子のみを有効にする方法を記載している。
